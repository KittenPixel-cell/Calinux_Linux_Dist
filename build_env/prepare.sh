...
if ! test $(id -u defaultuser) ; then

groupadd defaultuser
useradd -s /bin/bash -g defaultuser -m -k /dev/null defaultuser
passwd defaultuser
chown -v defaultuser $LFS/{usr,lib,var,etc,bin,sbin,tools,lib64,sources}

dbhome=$(eval echo "~defaultuser")
